...
const words = ["analyze", "optimize", "match", "elevate"];
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 py-32 lg:py-40">
        ...
        {/* Main headline */}
        <div className="mb-12">
          <h1 className={`text-[clamp(3rem,10vw,8rem)] font-display leading-[0.9] tracking-tight transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            ...
            <span className="block">
              ...
              <span className="relative inline-block">
                ...
                <span className="absolute -bottom-2 left-0 right-0 h-3 bg-violet-500/20 rounded" />
              </span>
            </span>
            <span className="block text-muted-foreground">elevated.</span>
          </h1>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-end">
          <p className={`text-xl lg:text-2xl text-muted-foreground leading-relaxed max-w-xl transition-all duration-700 delay-200 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            Upload your resume, discover skill gaps, match with jobs, and get personalized AI career advice — all powered by Llama 3.
          </p>
          ...
          <div className={`flex flex-col sm:flex-row items-start gap-4 transition-all duration-700 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <Button
              asChild
              size="lg"
              className="bg-violet-600 hover:bg-violet-700 text-white px-8 h-14 text-base rounded-full group"
            >
              <Link href="/register">
                Get started free
        ...
        <div className="flex gap-16 marquee whitespace-nowrap">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex gap-16">
              {[
                { value: "95%", label: "parse accuracy", company: "GROQ AI" },
                { value: "<3s", label: "analysis speed", company: "LLAMA 3" },
                { value: "100%", label: "free to start", company: "RESUMATE" },
                { value: "10k+", label: "professionals", company: "GROWING" },
              ].map((stat) => (
